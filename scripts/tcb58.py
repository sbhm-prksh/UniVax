...
# Purpose: TCL B58 supertype Epitope detection after doing Allergen, Antigen and Signal P Detection using NetCTL1.2 Server.
...
# Output: output_file variable. I have named it TC(B58).csv. Feel free to change. It contains all the epitopes which are strong binders.
# Output2: output_csv variable. I have named it filtered_TC(B58).csv. Feel free to change. Contains only those epitopes which  succeeded in allergen test, antigen(pending) test and toxicity test.
        ...
        if pre_tag and "Number of weak binders" in pre_tag.text:
            ...
            for line in lines:
                if "<= SB" in line:  # Identify lines with epitopes
                    parts = line.split()
                    epitope = parts[2]  # Assuming 2th column is Epitope
...
output_file = "./outputs/TC(B58).csv"  # OUTPUT CSV File. Update Accordingly.
...
print("------------------------------------------------------")
print(f"| Success: TC B58 Subtype Epitope Prediction.         |")
...
output_csv = './outputs/filtered_TC(B58).csv'  # OUTPUT CSV File. Change Accordingly.
...
total_time = end_time - start_time
print("------------------------------------------------------")
print(f"| Success: Detecting TC(B58) Cell Epitopes.           |")
